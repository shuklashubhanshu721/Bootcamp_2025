...
async def register_user(payload: RegisterRequest):
    ...
    try:
        ...
        active_db = database_module.get_db()
        if active_db is not None:
            existing_user = active_db["users"].find_one({"email": payload.email})
            if existing_user:
            ...
            result = active_db["users"].insert_one(new_user.dict(by_alias=True))
            return JSONResponse(
            ...
            hashed_pw = pwd_context.hash(payload.password)
            mock_users[payload.email] = hashed_pw
            mock_user = {
                "email": payload.email,
                "hashed_password": hashed_pw,
                "role": "agent",
...
async def login_user(payload: LoginRequest):
    ...
    active_db = database_module.get_db()
    if active_db is not None:
        user = active_db["users"].find_one({"email": payload.email})
        if not user:
        ...
        active_db["users"].update_one({"_id": user["_id"]}, {"$set": {"last_login_at": datetime.utcnow()}})
        token_data = {"sub": payload.email, "role": user.get("role", "agent")}
        access_token = create_access_token(data=token_data)
        return TokenResponse(access_token=access_token)
    else:
        # fallback mock verification using in-memory users
        if payload.email in mock_users:
            stored_hash = mock_users[payload.email]
            if pwd_context.verify(payload.password, stored_hash):
                token_data = {"sub": payload.email, "role": "agent"}
                access_token = create_access_token(data=token_data)
                return TokenResponse(access_token=access_token)
            raise HTTPException(status_code=401, detail="Invalid credentials")
        raise HTTPException(status_code=503, detail="Database unavailable and no mock users registered")


async def get_current_user(token: str = Depends(oauth2_scheme)):
    """Validate JWT and return current user"""
    from app.infrastructure.database.mongo_client import database_module
    live_db = database_module.get_db()
    if live_db is None:
    ...
    user = live_db["users"].find_one({"email": email})
    if user is None:
        raise credentials_exception
    return User(**user)
